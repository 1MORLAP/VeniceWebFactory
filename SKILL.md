...
8a. QA Gate (all options)  →  qa-check at mobile+ipad+desktop+desktop-wide for A, B, C (Phase O)
...
- **No empty stretched-click anchor overlays as flex siblings.** The "make-the-whole-card-clickable" pattern is fine in principle but Sonnet workers consistently misimplement it as `<a class="flex flex-col h-full" tabindex="-1" aria-hidden="true"></a>` placed as a flex-col sibling of the card content. That empty anchor takes layout space (h-full claims 100% of the cross axis) and produces visible empty cream / blank space inside the card — visible most clearly when content height varies between cards in a grid (the cards with less content show a giant gap above the photo). Real bug 2026-05-07 (johnsepticservice.com — 3 of 4 service cards on Option A and B had ~50% empty space above the photo because of this anti-pattern; spec didn't ask for stretched-click; Sonnet invented it). Photo `<a>` and "Learn more →" `<a>` already provide adequate click targets. **If a stretched-click is genuinely required**, implement as `<a class="absolute inset-0 z-10" aria-hidden="true" tabindex="-1"></a>` inside a `position: relative` parent — never as a flex sibling. qa-check's `empty-anchor-as-flex-child` rule (added 2026-05-07) verifies post-build.
...
1. **Stage 1 post-scrape** runs `scripts/classify-images.cjs <domain>` which tags every manifest image record with `_class: 'content' | 'nav-button' | 'banner' | 'line' | 'spacer' | 'tracking' | 'tiny' | 'icon'`. The classification is deterministic and runs before any image-pool generation. **Phase H (2026-05-06)**: classifies BOTH `pages[*].images[]` (foreground `<img>` tags) AND `pages[*].backgroundImages[]` (CSS `background-image` URLs). Pre-Phase-H this only walked `images[]`, leaving backgrounds unclassified — and downstream filters interpreted "no `_class`" as "not content," silently dropping every CSS-background photo. Real bug filed via lisastephenscpa.com 2026-05-06: the customer's team photo (`headertop.jpg`, on every page as a CSS background) was missing from all 3 deploys. The new chrome-keyword + bytes-per-pixel heuristic correctly distinguishes content (e.g., `headertop.jpg` at 0.43 bpp → content) from chrome (e.g., `header-bg.jpg` with `-bg` suffix → banner; `gradient-shadow.png` with chrome keyword → banner; low-bpp solid fills → banner).
2. **Stage 2 / 2.5 image-pool generation** MUST filter to `_class === 'content'` before assigning images to portfolio / catalog / gallery slots. Icon-class is allowed only for navigation surfaces; the other six classes never reach the page image-pool. Sub-agent prompts at Stage 3/5/7d MUST receive only content-class images for portfolio sections. **Per Phase H**: the per-page image pool MUST union BOTH `pages[*].images[]` and `pages[*].backgroundImages[]` filtered by `_class === 'content'`. Treating only foreground `<img>` records as the pool misses banner-shaped content imagery on Dreamweaver / GoDaddy / Wix / static-HTML legacy sites that put hero / team / banner photos as CSS backgrounds.
...
>
> **Pre-dispatch (Phase N.1, 2026-05-07)**: orchestrator runs `node scripts/extract-brief-input.cjs $DOMAIN` (slim manifest extract — Phase L.2) AND `node scripts/select-refero-styles.cjs $DOMAIN --for=a --n=5` (industry-relevant DESIGN.md inspiration priors from `templates/inspiration/refero-styles/`) before the brief sub-agent dispatch. Selector falls back to brief-input.json or manifest.json when design-brief.json doesn't exist yet. Brief sub-agent reads `~/.claude/skills/refero-design/SKILL.md` + `references/{anti-ai-slop,typography,color,motion}.md` (universal anti-slop guard, ALWAYS-LOAD) AND the priors file (industry inspiration, INSPIRATION-NOT-SPECIFICATION rule applies — see REFERO REFERENCES rule).
>
> **Inputs**: manifest.json, screenshots (visual read), brief-input.json, refero-style-priors-a.json (when non-empty), refero-design SKILL + references.
...
> **Outputs**: option-a passes qa-check at all 4 viewports (mobile, ipad, desktop, desktop-wide), punch-list cleared, `build-design-decisions.md` + `world-class-audit.md` written.
...
> **→ NEXT: Stage 5 — Build Option B.** Stage 4 (QA Option A) just completed — A passes qa-check at all 4 viewports (mobile + iPad + desktop + desktop-wide), Visual Sanity Pass done, World-Class Audit logged. Continue IMMEDIATELY to Stage 5. Do NOT ask the user "happy with A? ready for B?" Do NOT pause to compare. Just start B. (See PIPELINE COMPLETION CONTRACT at top.)
