...
Args (free-form prompt):
  ...
  6. REFERO STYLES PRIORS (added 2026-05-05 as Phase E — Option C only;
     pivoted from Refero MCP to LOCAL library 2026-05-07). Before
     committing the final design language, read the local DESIGN.md
     priors that the orchestrator pre-selected for this build:

       jobs/{domain}/refero-style-priors-c.json

     Generated by `scripts/select-refero-styles.cjs $DOMAIN --for=c` —
     reads `industry-tokens.json → direction`, filters the local
     `templates/inspiration/refero-styles/` library (1,226 brands —
     Tailwind v4 `@theme` DESIGN.md per brand) by keyword overlap +
     colorScheme alignment, penalizes SaaS/fintech bleed, returns the
     top 5 picks with their `path` to the full DESIGN.md file.

     For each pick in `entries[]`, read the `northStar` and reject
     obvious aesthetic mismatches (SaaS-metaphor matches, control-plane
     aesthetic, gradient-orb cliches — see anti-ai-slop.md). For the
     1-2 that survive the second-filter, Read the full DESIGN.md at
     `entries[i].path` (each ~10-15KB — color tokens + typography +
     spacing + components + do's/don'ts + agent prompt guide).

     Incorporate the STRUCTURAL ideas from accepted references:
     section order, hero composition, spacing rhythm, typographic
     hierarchy, component patterns. **DO NOT copy visual style
     verbatim** — those tokens belong to other brands. The
     `industry-tokens.json` palette + typography are the canonical
     visual style for this build.

     If `entries: []` is empty (no positive-score matches in the
     corpus for this customer's industry — common for SMB contractors
     since the corpus skews B2B SaaS / fintech / productivity), skip
     this step. Rely on `industry-tokens.json` + your chosen
     `templates/inspiration/<directory>/` instead. See REFERO
     REFERENCES rule in SKILL.md.

     **No network calls.** This step is fully offline. No MCP, no
     auth, no API key. The 2026-05-07 pivot from `mcp__refero__*`
     to the local library was for determinism + speed + cost.
