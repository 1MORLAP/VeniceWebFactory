...
1. Read every desktop and mobile screenshot under `jobs/{domain}/qa-option-{a|b|c}/`. **Phase L.1 (2026-05-07): prefer the `*.jpg` sidecars over `*.png`.** The orchestrator runs `scripts/compress-screenshots.cjs` before dispatching this sub-agent; that produces 1280px-wide JPEG-Q75 versions next to each PNG (typically ~96% smaller bytes, comparable visual signal at design-review resolution). Read the JPGs first; PNGs are the larger fallback when a JPG sidecar is missing. Both `desktop-*.{jpg,png}` and `mobile-*.{jpg,png}` apply.
2. Apply the 18-item checklist below — every item, every page, every viewport. Mobile FIRST per item #1.
3. For Stage 4c-bis / 7g only: pull industry-relevant references for the diversity check (item #18). **Phase E preferred path** (added 2026-05-05): use the Refero MCP — call `mcp__refero__refero_search_screens` with platform="web" and query like "{industry} services landing page" or "{industry-direction} brand site". Get 5-10 results, identify 2-3 that match the customer's industry (NOT generic SaaS — Refero's dataset skews B2B SaaS / fintech / productivity). Fetch screen content via `mcp__refero__refero_get_screen_content` for the chosen 2. **Fallback path** (legacy): read 1-2 peer-build homepage screenshots from other `jobs/{some-other-domain}/qa-option-{same-letter}/desktop-home.png` files. Use the legacy path when Refero results are all SaaS-clones for this customer's industry, OR when the orchestrator doesn't pass Refero references (smaller customer-specific peer set still works). See REFERO REFERENCES rule in SKILL.md for the dataset-bias caveat.
