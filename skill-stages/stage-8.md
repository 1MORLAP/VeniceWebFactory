...
**Verify after first deploy** (one-time sanity check per pipeline change): the build logs in the Vercel dashboard for any of these deployments should NOT contain the line `"Running 'vercel build'"`. Instead they should jump from `"Retrieving list of deployment files..."` to `"Using prebuilt build artifacts from .vercel/output"` to `"Deploying outputs..."`. If you see `"Running 'vercel build'"`, `--prebuilt` is being ignored — re-check that `.vercel/output/` exists in the option's root directory after `vercel build`, and that you're NOT passing `./dist` (or any path) to `vercel deploy`.

**Note on Build CPU Minutes** (Phase J, 2026-05-07): even with `--prebuilt` working correctly, Vercel allocates a Build Machine for every deploy to run the upload pipeline (you'll see `"Running build in iad1 (Turbo Build Machine)"` and `"Build machine configuration: 30 cores, 60 GB"` in the logs). This machine doesn't run our build — it just uploads files — but Vercel bills a 1-min minimum per deploy at the machine's per-minute rate. With 3 deploys/build × N builds/day, this scales painfully:
- ~$120/day at 100 builds/day on default Turbo class
- Switching the project's Build Machine class to **Standard** (4 cores, 8 GB) drops the per-minute rate. Vercel doesn't expose this field via the documented PATCH `/v9/projects/{id}` API (only `elasticConcurrencyEnabled` and `buildQueue` are PATCH-accepted there). Set manually per-project: **Vercel dashboard → Project → Settings → Build & Development → Build Machine → Standard**.
- `deploy-with-gate.cjs` passes `--archive=tgz` so the prebuilt output uploads as a compressed tarball — smaller upload = less billable time per deploy.

For radical cost reduction at 100+ builds/day scale, consider migrating customer-facing deploys to **Cloudflare Pages** (free tier: 500 builds/month, then $5/month flat — no per-minute build CPU cost). Vercel stays for our team's MCP-tooling stack. See FEEDBACK.md "Phase J — Vercel Build CPU Minutes investigation" for the full diagnosis + migration spec.
