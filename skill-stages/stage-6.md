...
node scripts/log-decision.cjs "$DOMAIN" 6c visual-pass-verdict --detail option=b --detail verdict="$VERDICT" --detail items_passed="$ITEMS_PASSED" --detail model=$VP_MODEL --detail effort=$VP_EFFORT
```
