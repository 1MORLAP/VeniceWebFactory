...
| 4.4 Logo content must contrast with immediate bg | The logo's PAINTED PIXELS (mean RGB of opaque pixels — `manifest.logo.dominantPaintedColor` from `fix-logo.js`) must reach WCAG ≥ 3:1 contrast vs the effective composited bg behind the logo (wrapper chip if any, else nav). White-on-transparent logo on cream chip = ~1.14:1, invisible. Dark-on-transparent logo on charcoal nav = same problem mirrored. Fix one of: (a) remove the wrapper element; (b) match wrapper bg to OPPOSITE polarity of logo paint; (c) swap to opposite-polarity logo variant if customer has one. | `logo-rendered-contrast` (FAIL — added 2026-05-07 via /webfactory-learn intake) |
