...
- **BILINGUAL SUPPORT (Options B and C, Spanish — added 2026-04-30)**: Options B and C ship `/es/` pages alongside English. Translation done ONCE in Stage 5 by the same Sonnet sub-agents that do the English rewrite (written to `option-b/src/pages/es/*.astro`); Option C reads B's `/es/` files at Stage 7 — single source of truth. Option A stays English-only (faithful rebuild contract). Translate page text + image alt + meta + nav labels; KEEP UNCHANGED phone/email/license/place-names/business-names/customer-attribution-names. Testimonials in `/es/`: translate body, append `(traducido del inglés)` tag below attribution, attribution name stays original. Every `/es/` page has `<html lang="es">`. Nav must include EN/ES toggle (`min-h-[44px]`, `aria-label`). qa-check enforces 4 new rules: `bilingual-page-parity`, `language-switcher-presence`, `html-lang-attribute`, `testimonial-tampering` extended for ES (compares C's ES against B's ES, requires the `(traducido del inglés)` tag). Per-build cost +25-40% vs English-only. See `BILINGUAL SUPPORT` in SKILL.md.
