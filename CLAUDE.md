...
- `scripts/select-refero-styles.cjs` — **Stage pre-step (added 2026-05-07)**: filters the local `templates/inspiration/refero-styles/` library (1,226 DESIGN.md files) by the customer's industry direction, writes a slim `jobs/{domain}/refero-style-priors-{a|c}.json` with up to 5 picks. Word-boundary keyword matching against each entry's `northStar`; +3 per unique industry keyword, +2 colorScheme bonus, -6 per SaaS/fintech penalty phrase. Default `--min-score=3`. Run by orchestrator BEFORE Stage 4c-tris audit + Stage 4c-bis/6c/7g visual-pass (`--for=a` or `--for=b`) and BEFORE Stage 7d Option C plugin (`--for=c`). Self-instruments via Phase F. Replaces the previous `mcp__refero__*` calls — no network dep on every build, deterministic, faster, no auth.
...
- `templates/scaffold/` — pure Astro 5 + Tailwind v4 scaffold (copied per-build, ZERO visual opinions). `templates/REQUIRED-PATTERNS.md` documents structural requirements every build must satisfy. `templates/inspiration/` is read-only references (saas-default, industrial-trades, industrial-trades-photo-led, more planned) AND **`templates/inspiration/refero-styles/`** (added 2026-05-07): a local mirror of styles.refero.design — 1,226 DESIGN.md files (Tailwind v4 `@theme` variant) per brand with color tokens, typography, spacing, components, do's & don'ts, agent prompt guide. Each ~10-15KB; canonical index at `_index.json`. Filtered per-build by `scripts/select-refero-styles.cjs` and consumed by Stage 4c-tris audit + Stage 4c-bis/6c/7g visual-pass diversity check + Stage 7d Option C plugin. Pivot from monolithic-template-copy to scaffold+inspiration architecture landed 2026-04-25 — see ROADMAP.md "Recently Shipped" + FEEDBACK.md.
...
- **REFERO REFERENCES (Phase E 2026-05-05; pivoted to local library 2026-05-07)**: Refero is now consumed via the on-disk mirror at `templates/inspiration/refero-styles/` — 1,226 DESIGN.md files (Tailwind v4 `@theme` variant) covering color tokens, typography, spacing, components, do's & don'ts, surfaces, elevation, imagery, layout, agent prompt guide per brand. **No network, no MCP, no API key** — fully offline-ready. Pre-selected via `scripts/select-refero-styles.cjs $DOMAIN --for={a|c}` which writes a slim `jobs/{domain}/refero-style-priors-{a|c}.json` with up to 5 industry-relevant picks; sub-agents read the priors file (small) instead of scanning the corpus. Wired into THREE places ONLY: Stage 7d (Option C plugin invocation) for industry-anchored DESIGN.md references, Stage 4c-tris (World-Class Audit) as Axis 3 of the audit's three world-class reference axes (post 2026-05-07 reshape — Axis 1 = Refero design.md taxonomy at `~/.claude/skills/refero-design/`, Axis 2 = `templates/inspiration/`, Axis 3 = `templates/inspiration/refero-styles/` priors), and Stage 4c-bis / 6c / 7g visual-pass diversity check (item #18) replacing hardcoded peer-build PNGs. **NEVER used for Stage 2 brief on Option A or per-page renders** — same dataset-bias caveat applies (corpus is the same content, just delivered locally; skews B2B SaaS / fintech / productivity, would amplify the editorial-drift failure mode for SMB contractor customers). Refero-using stages MUST gracefully degrade when priors file is empty (no positive-score matches — common for trades): fall back to `templates/inspiration/<dir>/` + peer-build PNGs. The Refero MCP (`https://api.refero.design/mcp`) is still installed for ad-hoc use outside the pipeline, but no pipeline stage depends on it. See REFERO REFERENCES rule in SKILL.md for the full architecture + filtering guidance.
...
- **MOBILE-FIRST DESIGN (all options)**: more than half of customer traffic is mobile. Design for 390×844 FIRST, scale up. Touch targets ≥ 44×44px (WCAG 2.5.5). No horizontal overflow at mobile viewport. Body text ≥ 16px. Sticky bottom-CTA recommended for trades. **qa-check runs at FOUR viewports since 2026-05-07 (Phase O)**: mobile (390×844 — primary, deploy-blocker), iPad (1024×1366 — md/lg breakpoint edge case), desktop (1440×900 — standard, deploy-blocker), desktop-wide (1920×1080 — FHD viewing). Reports per-issue tags: `[mobile-only]`, `[ipad-only]`, `[desktop-only]`, `[desktop-wide-only]`, multi-viewport combos like `[mobile+ipad]`, or `[all]` when fired at every viewport. Mobile-specific rules (`mobile-overflow`, `mobile-tap-target`, sticky-CTA-recommended, body-min-16px) stay scoped to mobile only — the new viewports add coverage at the upper end without diluting mobile primacy.
