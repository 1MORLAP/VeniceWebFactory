...
- **TEXT CONTRAST (generic, all options)**: qa-check walks every visible text element and computes WCAG contrast vs effective background. Fails at < 4.5:1 body / < 3:1 large. Catches the active-nav-black-on-black bug class regardless of which classes/colors caused it.
...
- **NEVER** use `preview_*` or `Chrome` MCP tools (`preview_start`, `preview_screenshot`, `mcp__Claude_in_Chrome__*`, `mcp__Control_Chrome__*`) — they show visible browser windows AND `preview_start` side-effect-creates `.claude/launch.json`. **Playwright itself is UNRESTRICTED** — use `scripts/qa.cjs` for the formal QA stages (consistent reporting) AND write ad-hoc Playwright scripts in `/tmp/*.mjs` for any iteration / debugging / single-component check at any pipeline stage. The ban is on visible-browser MCP tools, not on Playwright.
